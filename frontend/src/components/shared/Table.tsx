            ...
            <tr key={idx}>
              ...
              <td className="px-5 py-5 border-b border-gray-200 bg-white text-sm text-right">
                <div className="relative inline-block text-left" ref={el => dropdownRefs.current[idx] = el}>
                  ...
                  {dropdownOpen[idx] && (
                    <div className="origin-top-right absolute max-w-fit flex flex-end z-10 bg-white right-0 mt-2 w-56 rounded-md shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none" role="menu" aria-orientation="vertical">
                      <div className="py-1" role="none">
                        {dropdownOptions.map((option, optionIndex) => (
                          <a key={optionIndex} href="#" className="text-gray-700 block px-4 py-2 text-sm pr-8" role="menuitem" onClick={() => { option.action(item); toggleDropdown(idx); }}>
                            {option.label}
                          </a>
                        ))}
                      </div>
